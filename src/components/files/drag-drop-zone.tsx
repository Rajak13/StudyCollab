...
import { useDesktopFileEvents, useDesktopFileHandling } from '@/hooks/use-desktop-file-handling'
        ...
        const files: DragDropFile[] = await Promise.all(
          filePaths.map(async (filePath) => {
            try {
              ...
              const stats = await (window.desktopAPI as any)?.getFileStats?.(filePath)
  ...
  const uploadFiles = useCallback(async () => {
    ...
    for (const file of selectedFiles) {
      try {
        ...
        // Cache file locally
        if (cacheFile) {
          const content = await window.desktopAPI?.readFile?.(file.path)
